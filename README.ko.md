...
Collect는 지식 추출 엔진입니다. Claude 세션 JSONL을 읽고, 노이즈를 제거한 뒤, 의미 있는 대화를 LLM에 보내 남길 지식을 추려냅니다. 저장 전에는 로컬 의미 임베딩으로 기존 노트와 비교해 새 노트 생성, 기존 노트 병합, 중복 스킵 중 하나를 선택합니다. 중복 판단에 사용되는 모델은 `paraphrase-multilingual-MiniLM-L12-v2`(mean pooling, L2 정규화, 코사인 유사도)이며, 완전히 로컬에서 실행됩니다. 외부 API로 데이터가 나가지 않으며, 모델은 최초 1회 다운로드 후 `~/.cache/huggingface/`에 캐시됩니다(~125 MB). hooks가 설치되어 있으면 세션이 끝날 때마다 백그라운드에서 자동 실행됩니다. 직접 실행할 필요가 없습니다.

### Injection

세션 시작 시 `MY-PROFILE.md`를 읽고, 최근 노트를 최신성·신뢰도·현재 프로젝트 디렉터리와의 관련도로 채점해 상위 결과를 Claude의 `additionalContext`에 주입합니다. 프롬프트를 입력할 때는 동일한 로컬 임베딩 모델이 프롬프트와 볼트의 모든 노트 사이의 의미 유사도를 계산합니다. 키워드가 일치하지 않아도 의미가 가까운 노트가 첨부됩니다. Claude는 항상 당신이 누구인지, 무엇을 이미 알고 있는지 알고 있습니다.

### Explore

> *"나는 내가 모른다는 사실을 안다."* — 소크라테스

자신이 무엇을 모르는지 아는 것이 진짜 성장의 시작입니다. `explore`는 볼트에 쌓인 지식과 `MY-PROFILE.md`에 적힌 목표를 비교 분석해 4가지 유형의 갭 맵을 만들어냅니다:

- **Missing** — 목표에는 있는데 볼트에 노트가 없음
- **Shallow** — 개념 노트만 있고 트러블슈팅·패턴이 없음
- **Fragile** — 노트는 있지만 구조적으로 약함 (낮은 confidence, stale, 연결 없음)
- **Adjacent** — 목표에는 없지만 지금 상태에서 자연스러운 다음 단계

분석은 결정론적입니다. 같은 볼트 상태에서는 항상 같은 결과가 나옵니다. 목표 토픽은 프로필에서 LLM이 추출해 프로필 해시 기준으로 캐시됩니다. 볼트가 성장해도 목표가 바뀌지 않으면 추가 API 비용이 없습니다. LLM은 두 가지만 담당합니다: 자유형 목표 텍스트 → 구체 토픽 변환(캐시됨), 그리고 adjacent 추천 + 총평.
...
kore-chamber explore               # 갭 분석 (missing / shallow / fragile / adjacent)
kore-chamber explore --refresh     # 목표 토픽 재파싱 (캐시 초기화)
